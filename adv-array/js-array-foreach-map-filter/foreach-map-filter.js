...
function onlyEvenValues(arr){
    let evenValues = arr.filter( function(num){
        if ( num % 2 === 0 ) {
            return num;
        }
    });
...
function showFirstAndLast( arr ){
    ...
    arr.forEach( function( val ) {
        new.push( val[ 0 ] + val[ val.length - 1 ] );
    });
    return new;
}
...
// IMPORTANT: Please check doubleValues() at the top
...
*/
// I had not used the [0] in the end and think would be useful to have the index of the element.
