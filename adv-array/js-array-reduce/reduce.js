...
*/
// I was complicating things trying to create a new arr instead of using the accumulator
function extractValue(arr, key){
    return arr.reduce( function ( acm , next ){
      console.log( acm );
        acm.push( next[ key ] );
        return acm;
    },[]);
...
*/
// okay, need to read more about reduce returning arrays, I did a mess. :P
...
*/
// no, no, no, why use reduce here? I got how, but don't get why (same as previous comment)
function addKeyAndValue( arr , key , value ){
    return arr.reduce(function( acm , next , idx ){
        acm[idx][key] = value;
        return acm;
    },arr);
};
...
function partition(arr, cb){
    return arr.reduce(function(acc,next){
        /* true result goes to the 1 subarray and false result goes to the second */
        if(cb(next)){ 
            acc[0].push(next);
        } else {
            acc[1].push(next);
        ...
        return acc;
      // Reminder: this is the Initial value
    }, [[],[]]);
}